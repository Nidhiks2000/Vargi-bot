...
class Ur5Moveit:
    ...
    def __init__(self, arg_robot_name):
        ...
        rospy.init_node('process_packagen00', anonymous=True)
...
def main():
    ...
    ur5.moveit_hard_play_planned_path_from_file(ur5._file_path, 'homepose.yaml', 5)
    activate_conveyor_belt(100)

    def callback(msg):
        
        if msg.models[1].type == "packagen00":
            ...
            ur5.moveit_hard_play_planned_path_from_file(ur5._file_path, 'home_to_redbin.yaml', 1)
            activate_vacuum_gripper(False)
            ur5.moveit_hard_play_planned_path_from_file(ur5._file_path,'redbin_to_home.yaml',1)
            activate_conveyor_belt(100)
            or1 = ["1002", "Medicine", "1", "Hyderabad", "17.3850 N", "78.4867 E","HP","450"]
            or1.append(datetime.now())
            add = datetime.now()+timedelta(days = 1)
            or1.append(add)
            or1.append("Yes")
            ur5.update_google_sheets(or1)
            rospy.spin()
