...
class CartesianPath:
    ...
    def __init__(self):
        ...
        rospy.init_node('process_packagen3',anonymous = True)
    ...
    #functions for Rviz planning Scene
...
def main():
    ...
    lst_joint_angles = [math.radians(-100.163410991),
                        math.radians(-75.0595569657),
                        math.radians( 58.2813847498),
                        math.radians(-108.215114218),
                        math.radians( -89.9665333555),
                        math.radians(-7.80095885784)]
    ...
    def callback(data):
        ...
        #detecting packagen3
        if data.models[1].type == "packagen3" :

            ur5.add_box() #adding box to Rviz
            service.activate_conveyor_belt(9)

            #getting the translation value to grasp the package
            ...
            service.activate_vacuum_gripper(True) #activating vaccum
            ur5.attach_box() #attaching the box to arm in Rviz
            ur5.set_joint_angles(lst_joint_angles)  #go to bin
            service.activate_vacuum_gripper(False) #deactivating the package
            ur5.detach_box() #deattach the box from arm in Rviz
            ur5.remove_box()
            ur5.set_joint_angles(lst_home_pose)  #come back to home pose
            rospy.signal_shutdown("Blue process is done")
